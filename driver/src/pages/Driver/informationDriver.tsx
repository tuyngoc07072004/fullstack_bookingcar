...
import { Mail, Phone, User, FileText, Car, Lock, Save } from 'lucide-react'
import { driverApi } from '../../redux/Driver/Driver.Api'
...
interface InformationDriverProps {
  driverInfo: DriverInfo
  onProfileUpdated?: (info: DriverInfo) => void
}

const InformationDriver = ({ driverInfo, onProfileUpdated }: InformationDriverProps) => {
  const [name, setName] = useState(driverInfo?.name || '')
  const [phone, setPhone] = useState(driverInfo?.phone || '')
  const [licenseNumber, setLicenseNumber] = useState(driverInfo?.license_number || '')
  ...
  const handleRequestOtp = async () => {
    try {
      setSendingOtp(true)
      const result = await driverApi.requestPasswordChange({ method })
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      const result = await driverApi.verifyOtp(otpId, otp)
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      await driverApi.changePassword({ currentPassword, newPassword, verificationToken })
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100">
        ...
        <div className="bg-linear-to-r from-emerald-500 to-emerald-600 px-6 py-8 text-white">
          <div className="flex items-end gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold">{driverInfo?.name}</h1>
              {driverInfo?.status && (
        ...
        <div className="p-6 space-y-4">
          ...
          {driverInfo?.username && (
            <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-xl hover:bg-gray-100 transition-colors">
              ...
              <div className="flex-1">
                ...
                <p className="text-lg font-bold text-gray-900">{driverInfo.username}</p>
              </div>
            </div>
          )}
          ...
          <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-xl">
            ...
            <div className="flex-1">
              <p className=" text-gray-500 font-medium">Số Bằng Lái</p>
              <input
                className="w-full mt-1 px-3 py-2 rounded-lg border border-gray-200"
                value={licenseNumber}
          ...
          {driverInfo?.email && (
            <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-xl hover:bg-gray-100 transition-colors">
              ...
              <div className="flex-1">
                ...
                <p className="text-lg font-bold text-gray-900">{driverInfo?.email}</p>
              </div>
            </div>
          )}
